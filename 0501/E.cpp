...
{
    int p,r,org;
    bool operator <(const dept& b) const
    {
...
{
    int n,m;read(n,m);
    static int s[5050],q[5050];
    static dept d[5050];
    for (int i=1;i<=n;i++) read(s[i]),q[i]=i;
    for (int i=1;i<=m;i++) read(d[i].p),d[i].org=i;
    for (int i=1;i<=m;i++) read(d[i].r);
    sort(q+1,q+n+1,[&](int l,int r){return s[l]<s[r];});
    sort(d+1,d+m+1);
    static int ans[5050];
    for (int i=n;i;--i)
    {
        ...
        if (!x) {print("impossible");return 0;}
        d[x].p=0;
        ans[q[i]]=d[x].org;
    }
