...
signed main()
{
    ...
    while (T--)
    {
        ...
        readint(n,m);
        static rel r[200050];
        static bool happy[100050];
        for (int i=1;i<=m;i++)
            readint(r[i].x,r[i].y);
        static int sal[100050];int idx=0;
        while (idx<((m>>2)+1))
        {
            idx=0;
            for (int i=1;i<=n;i++) happy[i]=rand()&1;
            for (int i=1;i<=m;i++)
                if (happy[r[i].x] && !happy[r[i].y]) sal[idx++]=(i);
        }
        print(idx,'\n');
        for (int i=0;i<idx;i++) print(sal[i]," \n"[i==idx]);
    }
}