...
signed main()
{
    ...
    {
        if (!(N)) break;
        static vector<point> vp;vp.clear();
        int nneg=0;
        for (int i=0;i<N;i++)
        {
            point p;
            read(p.dx,p.dy);
            p.x=p.dx*2-W,p.y=p.dy*2-H;
            p.neg=(p.y<0)||(p.y==0 && p.x<0);
            if (p.neg)
            {
                p.x=-p.x,p.y=-p.y;
                ++nneg;
        ...
        int c;
        for (c=0;(2*nneg)^vp.size();c++)
        {
            if (vp[c].neg) --nneg;else ++nneg;
        }
        for (int i=0;i<vp.size();i++)
            if (!(vp[i].neg^(i<c))) print(vp[i].dx,' ',vp[i].dy,'\n');
    }
}