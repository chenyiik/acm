...
{
    int l,w,h;
    bool operator<(const nt& b) const {return w>b.w;}
};
signed main()
{
    int n,d;read(n,d);
    static nt q[(int)(1e5+50)];
    for (int i=1;i<=n;i++)
        read(q[i].l,q[i].w,q[i].h);
    sort(q+1,q+n+1);
    static int f[(int)(1e8+50)];
    int ans=0;
    for (int i=1;i<=n;i++)
    {
        if (f[q[i].w]+q[i].l>d) ++ans;
        for (int j=q[i].w;j<min(2*q[i].w,(int)1e8);j++)
            f[j-q[i].w]=max(f[j-q[i].w],f[j]+q[i].h);
    }
    print(ans,'\n');
}