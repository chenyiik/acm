...
signed main()
{
    static iint inv[6000050];
    inv[1]=1;
    const iint modl=998244353;
    for (iint i=2;i<6000050;i++) inv[i]=(modl-modl/i)*inv[modl%i]%modl;
    static iint ans[6000050];
    for (int i=1;i<6000050;i++) {ans[i]=ans[i-1]+inv[i]*inv[i]%modl;ans[i]%=modl;}
    for (int i=1;i<6000050;i++) {ans[i]*=inv[i];ans[i]%=modl;}
    int t;read(t);
    while (t--)
    {
        int n;
        read(n);
        println(ans[n]*3%modl);
    }
}