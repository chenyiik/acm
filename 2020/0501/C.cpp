...
{
    int n;read(n);
    static map<string,int> sc={{"r",1},{"y",2},{"g",3},{"br",4},{"blu",5},{"p",6},{"bla",7}};
    static int num[8];
    for (int i=1;i<=n;i++)
    {
        string s;
        cin>>s;
        if (s[0]!='b') s=s.substr(0,1);
        else if (s[1]!='l') s=s.substr(0,2);else s=s.substr(0,3);
        num[sc[s]]++;
    }
    int maxs;
    for (maxs=7;maxs;--maxs) if (num[maxs]) break;
    if (maxs==1) {print("1");return 0;}
    int ans=(maxs+1)*num[1];
