...
signed main()
{
    ...
    {
        int n;read(n);
        int nummax=n<<1;++nummax;
        static int vis[100050*2],presum[100050*2];
        memset(vis,0,sizeof(vis));
        int ans=0,numc=0;
        for (int i=1;i<=n;i++)
        {
            int tmp;read(tmp);
            if (tmp>nummax) {++ans;continue;}
            if (vis[tmp]) {++ans;continue;}
            ++vis[tmp];++numc;
        }
        if (!numc) {print(n+1,'\n');continue;}
        memset(presum,0,sizeof(presum));
        for (int i=1;i<=nummax;i++) presum[i]=presum[i-1]+vis[i];
        int tans=0x3f3f3f3f;
        for (int i=1;i<=nummax;i++) tans=min(tans,i+presum[nummax]-2*presum[i]);
        ans+=tans;ans=min(ans,n+1);
        print(ans,'\n');
    }
}