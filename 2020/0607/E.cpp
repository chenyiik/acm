...
iint n,m,d;
iint hp[3][10];
inline iint encode(iint my[],iint enemy[])
{
    iint code=0;
    for (iint i=1;i<=6;i++) code=code*10+hp[2][i];
    for (iint i=1;i<=6;i++) code=code*10+hp[1][i];
    return code;
}
unordered_map<iint,double> p;
double dfs(iint stat,iint d)
{
    if (p.count(stat)) return p[stat];
    if (stat<iint(1e6)) return 1;
    if (!d) return 0;
    iint headcount=accumulate(hp[1]+1,hp[1]+7,0LL)+accumulate(hp[2]+1,hp[2]+7,0LL);
    double res=0;
    for (iint i=1;i<=2;i++)
        for (iint j=1;j<=6;j++)
        if (hp[i][j])
        {
            hp[i][j]--;hp[i][j-1]++;
            auto nstat=encode(hp[1],hp[2]);
            double tmpl=dfs(nstat,d-1);
            p[nstat]=tmpl;
            hp[i][j]++;hp[i][j-1]--;
            res+=hp[i][j]*tmpl/headcount;
        }
    return res;
}
signed main()
{
    read(n,m,d);
    for (iint i=1;i<=n;i++) {iint t;read(t);++hp[1][t];}
    for (iint i=1;i<=m;i++) {iint t;read(t);++hp[2][t];}
    auto nstat=encode(hp[1],hp[2]);
    print<8>(dfs(nstat,d));print('\n');
}