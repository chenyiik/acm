...
{
    int n,m;readint(n,m);
    static int a[205][205],b[205][205];
    for (int i=1;i<=n;i++)
        for (int j=1;j<=m;j++) readint(a[i][j]);
    memset(b,0,sizeof(b));
    for (int j=1;j<=m;j++) b[1][j]=a[1][j];
    for (int i=2;i<=n;i++)
        for (int j=1;j<=m;j++) b[i][j]=a[i][j]+b[i-1][j];
    double gmaxv=0;
    for (int i=1;i<=n;i++)
    {
        double maxv=0;
        for (int j=1;j<=m;j++) maxv=max(maxv,double(b[i][j])/double(a[i][j]));
