...
void doo()
{
    static stack<pair<int,int>> num[102450];
    int n,k;read(n,k);
    int all=(1<<k)*n;
    for (int i=all;i>=1;i--)
    {
        while (num[i].size()) num[i].pop();
        int u,v;read(u,v);
        num[i].push({u,v});
    }
    while (k--)
    {
        for (int i=all;i>=1+(all>>1);i--)
        {
            int aim=all-i+1;
            while (num[i].size())
            {
                auto t=num[i].top();num[i].pop();
                num[aim].push({t.second,t.first});
            }
        }
        all>>=1;
    }
    for (int i=n;i>=1;i--)
    {
        static vector<pair<int,int>> vec;vec.clear();
        while (num[i].size()) {vec.push_back(num[i].top());num[i].pop();}
        for (auto x:vec) print(x.first,' ');
        for (auto x:vec)
            print(x.second," \n"[i==1 && x==vec.back()]);
    }
}
