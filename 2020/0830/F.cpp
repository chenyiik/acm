...
void doo()
{
    static int l[100050],r[100050];
    int n,k;read(n,k);
    for (int i=1;i<=n;i++) read(l[i],r[i]);
    for (int i=1;i<n;i++) l[i+1]=max(l[i+1],l[i]-k),r[i+1]=min(r[i+1],r[i]+k);
    for (int i=n;i>1;i--) l[i-1]=max(l[i-1],l[i]-k),r[i-1]=min(r[i-1],r[i]+k);
    bool fail=0;
    for (int i=1;i<=n;i++) if (l[i]>r[i]) {fail=1;break;}
    if (fail) {println("NO");return;}
    println("YES");
